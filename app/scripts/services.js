                        ...
                        angular.forEach(data.results, function(result) {
                            angular.forEach(result.patient.drug, function(drug) {
                                if (drug.openfda && drug.openfda.substance_name) {
                                    angular.forEach(drug.openfda.substance_name, function(substanceName) {
                                        if (angular.uppercase(substanceName).includes(ucname)) {
                ...
                $q.all([eventPromise, labelPromise, recallPromise]).finally(function() {
                    if (error && drugs.length === 0) {
                        ...
                        angular.forEach(uniqueDrugs, function(drug) {
                            console.log('name: ' + drug.substance_name);
                        });
                        deferred.resolve({
                            meta: {
                                results: {
